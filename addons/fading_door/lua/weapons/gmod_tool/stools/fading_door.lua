...
if CLIENT then
	...
	language.Add("Tool.fading_door.desc", "Permets de faire des portes facilement")
	language.Add("Tool.fading_door.0", "Cliquez sur quelque chose pour en faire une porte. Faites un clique droit pour copier les données. Rechargez pour enlever la porte.")
	
	function TOOL:BuildCPanel()
		self:AddControl("Header",   {Text = "#Tool.fading_door.name", Description = "#Tool.fading_door.desc"})
		self:AddControl("CheckBox", {Label = "Inversé", Command = "fading_door_reversed"})
		self:AddControl("CheckBox", {Label = "Reste Active", Command = "fading_door_swap"})
		...
		local left = vgui.Create("DLabel", self)
		left:SetText("Son de fermeture")
		left:SetDark(true)
		DoorCloseSound:SetHeight(25)
		DoorCloseSound:Dock(TOP)
		
		self:AddItem(left, DoorCloseSound)
		self:AddControl("Numpad", {Label = "Accès", ButtonSize = "22", Command = "fading_door_key"})

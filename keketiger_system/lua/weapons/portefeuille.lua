...
if SERVER then
    util.AddNetworkString("PortefeuilleOpen")
end
...
if CLIENT then
    ...
    local function theDerma()
        ...
        if not IsValid(ply) or not ply:Alive() or IsValid(FMain) then return end
        ...
        local FMain_Text = vgui.Create("DTextEntry", FMain)
            ...
            FMain_Text.AllowInput = function( self, stringValue ) if string.find(stringValue,"%d") then
            return false
        end
            return true
        end
        ...
        local FMain_Give = vgui.Create("DButton", FMain)
            ...
            FMain_Give.DoClick = function() local value = FMain_Text:GetValue() if value and type(value) == "string" and value ~= ""
                then
                    ply:ConCommand("say /give "..value) if IsValid(ply:GetEyeTrace().Entity)
                        and ply:GetEyeTrace().Entity:IsPlayer()
                then
                    chat.AddText( color_white, "Vous avez donné "..DarkRP.formatMoney(tonumber(value)).." à "..ply:GetEyeTrace().Entity:Name()..".")
                end
                    FMain:Close(true)
            end
        end  
        ...
        FMain_Drop.DoClick = function() local value = FMain_Text:GetValue() if value and type(value) == "string" and value ~= "" 
            then
